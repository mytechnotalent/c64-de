...
#   ./build.sh debug    Build and launch in VICE with debug
#                        symbols loaded
# ---------------------------------------------------------
...
# Auto-detect architecture for VICE
...
echo -e "${CYAN}=== C64 Sprite Demo Build ===${NC}"
echo "Source: $SRC"
echo ""
...
if [ "$1" = "debug" ]; then
    echo ""
    echo -e "${CYAN}Launching in VICE (x64sc) with debug symbols...${NC}"
    SYM_FILE="$SCRIPT_DIR/main.sym"
